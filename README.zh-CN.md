...
- **修复前**：只有 `重要提示。` 显示加粗——`GitHub` 被压回正常字重。
- **修复后**：**两处都正确显示加粗**。

**根因**：插件收集"正确加粗"时，只接受**匹配内容本身含中文/全角标点**的匹配（`isCJKRelated(inner)`）。`**GitHub**` 这类纯英文内容被直接跳过；当解析器因 CommonMark flanking 问题把整行误判为加粗时，插件的"纠正网"（第 3 阶段）把整行除第一处中文加粗外的内容全部压回正常字重，英文加粗被一并误伤。
